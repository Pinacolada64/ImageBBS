...
; 30/Jul/2016 18:49
; now using Jeff's casm 4.0 beta3 assembler
...
keepbuf:
		...
		sta case1		; 102b:002d	8d f4 03
		jmp goto1048		; 102e:0030	4c 48 10

clearbuf:
		lda case		; 1031:0033	ad 01 d0
		sta case1		; 1034:0036	8d f4 03
		jsr sub_1ab4		; 1037:0039	20 b4 1a
		lda #$01		; 103a:003c	a9 01
		sta cline		; 103c:003e	8d fb 03
		sta mline		; 103f:0041	8d f8 03
goto1042:
...
fwd_1084:
; enter chat mode while in editor?
...
dotaddrs:
		word pAbort		; .A $1267
		word pBorder		; .B $1271
		word pColumns		; .C $1591
		word pDelete		; .D $12df
		word pEdit		; .E $133e
		word pFind		; .F $1351	; 115f:0161
		word pHelp		; .H $141b
		word pInsert		; .I $1423
		word pJustify		; .J $143d
		word pFindAndReplace	; .K $139c
		word pList		; .L $14f1
		word pMCIread		; .M $14fe	; 116b:016d
		word pNewText		; .N $150b
		word pLineNumbers	; .O $1566
		word pRead		; .R $1515
		word pSaveText 		; .S $1540
		word pVersion		; .V $169c
		word pExit		; .X $154b	; 1177:0179
		word pMove		; .Y $160c
		word pCopy		; .Z $15bf
		word pHelp		; .? $141b
		word pScale		; .# $1659
...
dottext:
		word szAbort		; .a $11bb	; 118d:018f	tbl-11ba
		word szBorder		; .b $11c1
		word szColumns		; .c $1240
		word szDelete		; .d $11c8
		word szEdit		; .e $11cf
		word szFind		; .f $11d4
		word szHelp		; .h $11d9
		word szInsert		; .i $11df
		word szJustify		; .j $11e6
		word szReplace		; .k $11fe
		word szList		; .l $1206
		word szMCIread		; .m $120b	; 1199:019b
		word szClearText	; .n $1214
		word szLineNumbers	; .o $1233
		word szRead		; .r $121f
		word szSaveText		; .s $1224
		word szVersion		; .v $125f
		word szExit		; .x $122e	; 11a5:01a7
		word szMove		; .y $124d
		word szCopy		; .z $1248
		word szHelp		; .<Tab>  $11d9
		word szScale		; .# $1252
		word szIndentOrphan	; .<Tab>  $1258	; 11b1:01b3

szAbort:
  		byte upper+$01		; 11bb:01bd	tbl-11c0
  		ascii "BORT"
		byte $00

szBorder:
		byte upper+$02		; 11c1:01c3	tbl-11c7
		ascii "ORDER"
		byte $00

szDelete:
		byte upper+$04		; 11c8:01ca	tbl-11ce
		ascii "ELETE"
		byte $00

szEdit:
		byte upper+$05		; 11cf:01d1	tbl-11d3
		ascii "DIT"
		byte $00

szFind:
		byte upper+$06		; 11d4:01d6	tbl-11d8
		ascii "IND"
		byte $00

szHelp:
		byte upper+$08		; 11d9:01db	tbl-11de
		ascii "ELP!"
		byte $00

szInsert:
		byte upper+$09		; 11df:01e1	tbl-11e5
		ascii "NSERT"
		byte $00
...
szJustify:
		byte upper+$0a
		ascii "USTIFY ("
		byte upper+$03,comma,upper+$05,comma,upper+$09,comma,upper+$0c,comma
		byte upper+$10,comma,upper+$12,comma,upper+$15
					; "C,E,I,L,P,R,U"
		ascii ")"
...
szMCIread:
		byte upper+$0d,upper+$03,upper+$09,space,upper+$12 ; "MCI R"
		ascii "EAD"
...
szExit:
		byte upper+$05		; 122e:0230	tbl-1232
		ascii "XIT"
		byte $00

szLineNumbers:
		byte upper+$0c		; 1233:0235	tbl-123f
		ascii "INE "
		byte upper+$0e
		ascii "UMBERS"
		byte $00
...
szColumns:
		byte upper+$03		; 1240:0242	tbl-1247
		ascii "OLUMNS"
		byte $00

szCopy:
		byte upper+$03		; 1248:024a	tbl-124c
		ascii "OPY"
		byte $00

szMove:
		byte upper+$0d		; 124d:024f	tbl-1251
		ascii "OVE"
		byte $00

szScale:
		byte upper+$13		; 1252:0254	tbl-1257
		ascii "CALE"
		byte $00
...
szIndentOrphan:
		byte upper+$09		; 1258:025a	tbl-125e
		ascii "NDENT"
		byte $00

szVersion:
		byte upper+$16		; 125f:0261	tbl-1266
		ascii "ERSION"
		byte $00
...
szCharacter:
		byte ImageReturn	; 12d0:02d2	tbl-12d0
		byte upper+$03
		ascii "HARACTER"
		byte colon
...
pDelete:
		ldx mline		; 12df:02e1	ae f8 03
		dex 			; 12e2:02e4	ca
		ldy mline		; 12e3:02e5	ac f8 03
		dey 			; 12e6:02e8	88
...
fwd_1300:
		...
		ldx #<szTo		; 130e:0310	a2 2d
		ldy #>szTo		; 1310:0312	a0 13
...
szTo:
		ascii " "
		byte upper+$14		; 132d:032f
		ascii "O "
		byte $00

szDeleted:
		ascii " "
		byte upper+$04		; 1332:0334
		ascii "ELETED.)"

		byte ImageReturn,$00	; 133c:033e

		; .Edit line (or range of lines)
pEdit:
		ldx mline		; 133e:0340	ae f8 03
		dex 			; 1341:0343	ca
		ldy mline		; 1342:0344	ac f8 03
		dey 			; 1345:0347	88
...
pFind:
		...
		ldy mline		; 1353:0355	ac f8 03
		dey 			; 1356:0358	88
		beq fwd_138a		; 1357:0359	f0 31
		jsr sub_1c8f		; 1359:035b	20 8f 1c
		ldx #<szSearchPhrase	; 135c:035e	a2 8d
		ldy #>szSearchPhrase	; 135e:0360	a0 13
...
pFindAndReplace:
		...
		ldy mline		; 139e:03a0	ac f8 03
		dey 			; 13a1:03a3	88
		beq fwd_13fa		; 13a2:03a4	f0 56
		jsr sub_1c8f		; 13a4:03a6	20 8f 1c
		ldx #<szSearchPhrase	; 13a7:03a9	a2 fd
		ldy #>szSearchPhrase	; 13a9:03ab	a0 13
...
pInsert:
		...
		cpx mline		; 142c:042e	ec f8 03
		bcs fwd_143c		; 142f:0431	b0 0b
		stx cline		; 1431:0433	8e fb 03
...
szCenter:
		byte upper+$03		; 147b:047d	tbl-1481
		ascii "ENTER"
		byte $00

szExpand:
		byte upper+$05		; 1482:0484	tbl-1488
		ascii "XPAND"
		byte $00

szPacked:
		byte upper+$10		; 1489:048b	tbl-148f
		ascii "ACKED"
		byte $00

szLeft:
		byte upper+$0c		; 1490:0492	tbl-1494
		ascii "EFT"
		byte $00

szRight:
		byte upper+$12		; 1495:0497	tbl-149a
		ascii "IGHT"
		byte $00

szIndent:
		byte upper+$09		; 149b:049d	tbl-14a1
		ascii "NDENT"
		byte $00

szUnindent:
		byte upper+$15		; 14a2:04a4	tbl-14ab
		ascii "N"
		byte $2d,upper+$09	; "-I"
		ascii "NDENT"
		byte $00
...
justaddr:
		...
		word pJustifyIndent	; $181a
					; 14bb:04bd	tbl-14c8
		word pJustifyUnindent	; $181f
					; 14c7:04c9
...
pMCIread:
		...
		sta flag1		; 1505:0507	8d f3 03
		jmp goto151f		; 1508:050a	4c 1f 15

		; .Clear Text (TODO: want confirmation!)
pNewText:
	; keep this 10 bytes:
	; check whether Return is hit:
	...
	; 10 bytes (moved to NewTextPatch):
;		jsr sub_ReturnKey	; 150b:050d	20 a7 1a
...
pRead:
		...
		sta flag1		; 151c:051e	8d f3 03
goto151f:
		ldx #$01		; 151f:0521	a2 01
		ldy mline		; 1521:0523	ac f8 03
		dey 			; 1524:0526	88
...
pExit:
		...
		lda mline		; 1552:0554	ad f8 03
		sta cline		; 1555:0557	8d fb 03
...
fwd_15fa:
		lda mline		; 15fa:05fc	ad f8 03
		sta cline		; 15fd:05ff	8d fb 03
...
fwd_1647:
		lda mline		; 1647:0649	ad f8 03
		sta cline		; 164a:064c	8d fb 03
...
pScale:
		...
		;	the caller is in 80 column mode - check "llen")
		;	"          1         2         3"
...
l_scale:
		...
		; seems to be a waste of space (40 bytes, could be 10
		; with 3-6 JSRs once .Scale string is dependant on 40/80
		; columns)
t_scale2:
...
t_169b:
		byte ImageReturn	; 169b:069d	tbl-169b
...
szEditorVersion:
		byte ImageReturn	; 16a5:06a7

		; todo: build date repeated, use sBuildDate instead?
...
pJustifyExpand:
		jsr pJustifyPacked	; 17d9:07db	20 9d 17
b_17dc:
...
; FIXME: why the repeated lda / sta here?
; seems wasteful - could save 6 bytes
...
sub_19d0:
		...
		dec mline		; 19e4:09e6	ce f8 03
		dec cline		; 19e7:09e9	ce fb 03
		rts 			; 19ea:09ec	60

goto19eb:
		stx arisgn		; 19eb:09ed	86 6f
		cpx mline		; 19ed:09ef	ec f8 03
		bcs fwd_1a0d		; 19f0:09f2	b0 1b
		ldx mline		; 19f2:09f4	ae f8 03
...
fwd_1a0d:
		...
		ldx cline		; 1a1a:0a1c	ae fb 03
		cpx arisgn		; 1a1d:0a1f	e4 6f
		bcc fwd_1a24		; 1a1f:0a21	90 03
		inc cline		; 1a21:0a23	ee fb 03
fwd_1a24:
		inc mline		; 1a24:0a26	ee f8 03
fwd_1a27:
		rts 			; 1a27:0a29	60
...
; get ready to print "Command:Exit"
		lda len_CmdExit		; 1a64:0a66	ad 94 1a
...
sCmdExit:
		...
		byte upper+$05 ; e
		ascii "XIT"
		byte ImageReturn	; 1aa1:0aa3	tbl-1aa1
...
sub_1af1:
		...
		ldx cline		; 1afc:0afe	ae fb 03
		cpx mline		; 1aff:0b01	ec f8 03
...
szNoRAM:
		...
		ascii "ELP."
					; 1c53:0c55
		byte ImageReturn,$00	; 1c57:0c59	tbl-1c58
...
sub_getline:
		...
		lda modes		; 1d31:0d33	ad ff 03
		and #$08		; 1d34:0d36	29 08
		ora #{%:01000101}	; 1d36:0d38	09 45
		tax 			; 1d38:0d3a	aa
...
DoAbort:
{ifdef:debug}
; works fine if .New Text not selected beforehand - then it requires
; .Abort hit 2x - must be addresses pushed on stack somewhere
		lda #$00
		sta $d020		; turn border black - means we got here
{endif}
		lda #$00		; 126a:026c	a9 00
		ldx #$00		; 126c:026e	a2 00
		jmp editrts		; 126e:0270	4c 09 10
...
; {ifdef:debug}
;		pha
;		jsr sub_outChar		; do it the Image way
;		pla
; {endif}
...
szAborted:
		...
		align $100,$ea		; fill with $ea to $1eff

	; must be a patch:
	; not sure why it needs to be on a page boundary,
